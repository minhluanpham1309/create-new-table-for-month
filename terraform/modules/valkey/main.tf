...
resource "aws_cloudwatch_metric_alarm" "valkey_cpu" {
  ...
  alarm_name          = "${var.project_name}-valkey-node-0001-high-cpu"
  ...
  alarm_description   = "Valkey node 0001 CPU utilization is too high"
...
resource "aws_cloudwatch_metric_alarm" "valkey_memory" {
  ...
  alarm_name          = "${var.project_name}-valkey-low-freeable-memory"
  ...
  alarm_description   = "Valkey freeable memory is critically low (OOM risk)"
  alarm_actions       = var.alarm_actions
