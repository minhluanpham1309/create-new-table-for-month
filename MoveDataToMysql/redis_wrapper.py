...
def _dec(raw: Optional[Union[str, bytes]]) -> Optional[str]:
    ...
    """
    if raw is None:
        logger.info("Redis value is None in _dec")
        return None

    try:
        text = raw.decode("utf-8") if isinstance(raw, bytes) else raw
    except UnicodeDecodeError:
        logger.error("Failed to decode Redis value as UTF-8; raw=%r", raw)
        return None
    ...
    try:
        ...
        logger.error(
            "Unexpected error while decoding Redis value; raw=%r; text=%r",
            raw,
            text
        )
        return text
...
class RedisWrapper:
    ...
    def __init__(
        ...
        pool = redis.ConnectionPool(
            ...
            retry=Retry(ExponentialBackoff(cap=20), retries=10),
