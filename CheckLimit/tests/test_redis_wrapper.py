...
from redis_utils import RedisWrapper, _enc, _dec
...
def wrapper(mock_client):
    with patch("redis_utils.redis.ConnectionPool"), \
         patch("redis_utils.redis.Redis", return_value=mock_client):
...
def fake():
    ...
    with patch("redis_utils.redis.ConnectionPool"), \
         patch("redis_utils.redis.Redis", return_value=client):
...
class TestInit:
    def test_pool_args(self):
        ...
        with patch("redis_utils.redis.ConnectionPool") as mock_pool, \
             patch("redis_utils.redis.Redis", return_value=mock_client):
    ...
    def test_retry_config(self):
        ...
        with patch("redis_utils.redis.ConnectionPool") as mock_pool, \
            ...
            assert kwargs["retry"]._retries == 5
...
# ============================================================================
# Retry trong ConnectionPool — test Retry.call_with_retry trực tiếp
#
# redis-py retry xảy ra bên trong ConnectionPool.execute_command():
#   pool.retry.call_with_retry(do, fail)
#
# Test trực tiếp Retry object (cùng object được config trong RedisWrapper)
# với NoBackoff() để không bị sleep.
# ============================================================================
...
class TestConnectionPoolRetry:
    ...
    def test_succeeds_after_transient_failures(self):
        """Fail 3 lần → retry → thành công (DNS update sau vài retry)."""
    ...
    def test_raises_after_all_retries_exhausted(self):
        """Fail liên tục đến hết retries → raise lên caller → Lambda fail → external retry."""
    ...
    def test_retries_on_timeout_error(self):
        """TimeoutError cũng được retry như ConnectionError."""
        retry = self._make_retry()
        call_count = 0
...
#
# Per-invocation pattern: mỗi lần tạo RedisWrapper mới = DNS resolve mới
# ============================================================================

class TestFailoverPrimaryDiesReplicaPromoted:
    def _make(self, fake_client):
        with patch("redis_utils.redis.ConnectionPool"), \
             patch("redis_utils.redis.Redis", return_value=fake_client):
